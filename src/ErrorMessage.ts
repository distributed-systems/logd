...
export default class ErrorMessage {
    ...
    */
    truncatePath(filepath: string) : string {
        ...
        if (filepath.startsWith(rootPath)) filepath = filepath.substring(rootPath.length+1);
        else if (filepath.startsWith(`file://${rootPath}`)) filepath = filepath.substring(rootPath.length+1+7);

        // check for node modules, remove that
        const index = filepath.lastIndexOf('node_modules');
        if (index >= 0) filepath = 'nm:'+filepath.substring(index+'node_modules'.length);

...
export default class Logd {
    ...
    getEnabledLogLevels() {
        ...
        process.argv.forEach((arg) => {
            ...
            if (parts) {
                let level;
                // @ts-ignore
                if (parts[4] && this.logLevels.some(l => l.name === parts[4]))
                    level = this.logLevels.find(l => l.name === parts[4]).level;
                if (parts[3])
                ...
                if (level !== undefined) {
                    ...
                    throw new Error(`Failed to determine loglevel, accepting level, level+, level-, number, got ${arg}`);
            }
        });
        // @ts-ignore
    ...
    */
    level(name, level = 50) {
        ...
        // notify all children
        for (const moduleItem of this.pathMap.values()) {
            moduleItem.addLoglevelHandler(levelDefinition, this.instance);
        }
        // add to collection
    ...
    */
    module(moduleName, dontBustCache = false) {

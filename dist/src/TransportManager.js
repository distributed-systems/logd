export default class TransportManager {
    ...
    addLogLevelDescriptor(descriptor) {
        if (!this.loglevels.has(descriptor.name))
            this.loglevels.set(descriptor.name, new Set());
                ...
                case 'open-range':
                    if (this.loglevels.has(level.name)) {
                        for (const levelNumber of this.loglevels.get(level.name)) {
                            if (level.modifier === '+') {
                                for (let i = levelNumber; i <= 100; i++)
                                    this.addTransportToLevel(transport, i);
                            }
                            else {
                                for (let i = levelNumber; i >= 0; i--)
                                    this.addTransportToLevel(transport, i);
                            }
                        }
                    }
    ...
    addTransportToLevel(transport, levelNumber) {
        if (!this.levelMap.has(levelNumber))
            this.levelMap.set(levelNumber, new Set());
